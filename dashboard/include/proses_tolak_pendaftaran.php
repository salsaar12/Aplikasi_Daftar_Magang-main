...
if (isset($_GET['idd']) && isset($_GET['reason'])) {
    $id = $_GET['idd'];
    $idu = $_GET['idu'];
    $idd = $_GET['idd'];
    $reason = $_GET['reason'];
    ...
    // Update the query to include the reason for rejection
    $query = "UPDATE detail_pendaftaran 
              SET status_pendaftaran = 2, id_admin = $idu, reason_rejection = '$reason' 
              WHERE id_user = $id";
    ...
    if ($exec) {
        $_SESSION['message'] = "1";
        echo '<script>
                alert("Pendaftaran telah ditolak dengan alasan: ' . $reason . '");
