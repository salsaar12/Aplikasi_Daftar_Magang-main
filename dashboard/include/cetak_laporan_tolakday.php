...
if (isset($_POST['tampil'])) {
    ...
    $pdf->Cell(100, 10, 'List Pendaftaran DiTolak Dari Tanggal ' . $daritgl . ' Sampai Tanggal ' . $ketgl . '', 0, 1, 'C');
    $pdf->Ln();
    ...
    $query  =  "SELECT * FROM pendaftaran a, akun b, detail_pendaftaran c WHERE a.Id = c.id_user 
                AND b.id_user = a.Id AND c.status_pendaftaran = 2 AND c.tanggal_daftar BETWEEN '$daritgl' AND '$ketgl'  ";
...
while ($rows = mysqli_fetch_array($exec)) {
    $pdf->Row(array(++$no, $rows['nama'], $rows['jenis_kelamin'], $rows['telepon'], $rows['tanggal_daftar'], $rows['nama_akademik'], $rows['jurusan_akademik'], "Ditolak"));
}
